...
def main() -> None:
    ...
    if len(config.apps) == 1 or config.ci_test:
        ...
        for app_name in config.apps:
            app_updates = process_single_app(app_name, config, download_cache, resource_cache)
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_app = {
                executor.submit(process_single_app, app_name, config, download_cache, resource_cache): app_name
                for app_name in config.apps
            }
